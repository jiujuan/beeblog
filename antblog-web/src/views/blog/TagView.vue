...
async function load() {
  ...
  try {
    ...
    tag.value = await tagApi.getBySlug(slug)
    const res = await articleApi.list({ tag_id: tag.value.id, page: page.value, page_size: pageSize.value })
      ...
      <main>
        <div class="mb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <h1 class="font-serif text-3xl font-semibold">{{ tag?.name ?? '…' }}</h1>
          </div>
        </div>
